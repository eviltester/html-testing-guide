---
links:
    - https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input/number
cssSelector: 'input[type="number"]'
ideas:
    - Number may include characters you don't expect, try them e.g. "e", ".","-","+"
    - Try changing the type to "text" and submitting a very invalid input.
---

import {UsefulLinks, TestIdeas, CSSSelectors} from '@site/src/components/ElementMetadata';

# `type="number"`

Number is not restricted to positive Integer so it is possible to add characters that the server or follow on functionality is not expecting e.g. "-.e" so we need to try the full range of input characters.

```
<input type="number" value="10"/>
```

<input type="number" defaultValue="10"/>



## Exercises

- What can you enter in the [Test Pages HTML5 Form](https://testpages.herokuapp.com/styled/html5-form-test.html) as a number which might cause validation errors on the server side?
    - https://testpages.herokuapp.com/styled/html5-form-test.html

## Useful Links

<UsefulLinks data={frontMatter}></UsefulLinks>

## CSS Selector

<CSSSelectors data={frontMatter}></CSSSelectors>

## Test Ideas

<TestIdeas data={frontMatter}></TestIdeas>




